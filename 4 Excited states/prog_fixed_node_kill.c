...
#define stepsperblock 1000 // controls how often the walker distribution is saved in a text file
...
double V_pot(long int D, long int tipo, double K_el, double *Vpar, double *x);                    //calculates the potential felt by the i-th walker
void Propagazione(long int D,long int N, double x_node, double **w, double tau);            //each walker does a random walk step
...
void f_pot(long int Nbin, double Rmax, long tipo, double K_el, double* Vpar, double *pot); //it is used to generate a normalised histogram to represent the wavefunction

//to compile: gcc -lm prog_fixed_node_kill.c -o prog_fixed_node_kill
...
int main(){
  ...
  double **w, **wnew, x_node=0.0, K_el=1.0;
  ...
  if (flag==1) {
    ...
    if (D==1) {
      ...
      f_pot(Nbin,Rmax,tipo, K_el, Vpar,pot);  //fill "pot" with the Nbin values of the potential and then with Nbin associated positions
      for( i=0; i<Nbin; i++)
        fprintf(fpot,"%lf\t%lf\n",pot[i+Nbin],pot[i]);
      fprintf(fpot,"\n");
      fclose(fpot);
  ...
  for( n=0; n<Nsteps; n++ ){
    ...
    for(i=0;i<Nwt;i++){
      if(*(w+i)!=NULL){
        A[i]=V_pot(D, tipo, K_el, Vpar, w[i]);
      }
    }

    if(flag==1){
      delta=Rmax/(double)Nbin;
      if((n % stepsperblock)==0){  //print the wavefunction during the entire simulation
    ...
    Propagazione(D,Nwt,x_node,w,tau);          //Diffusion step

    B=(double*) malloc (Nwt*sizeof(double));
    for(i=0;i<Nwt;i++){
      if(*(w+i)!=NULL){
        B[i]=V_pot(D, tipo, K_el, Vpar, w[i]);
      }
    }
...
double V_pot(long int D, long int tipo, double K_el, double *Vpar, double *x){
  double a=0;
  long int j;
...
void Propagazione(long int D,long int Nwt, double x_node, double **w, double tau){		//diffusion function
  long int i,j;
  double w_temp;
  for(i=0;i<Nwt;i++){
    for(j=0;j<D;j++){
      w_temp=w[i][j];		//save the previous walker position
      w[i][j]+= sqrt(tau)*norm(0,1);
      if (w_temp<x_node && w[i][j]>x_node)		// node-crossing check in one dimension; look at the two-fermion system code for a more general approach
        w[i]=NULL;
      else if (w_temp>x_node && w[i][j]<x_node)
        w[i]=NULL;
    }
  }
}
...
void f_pot(long int Nbin, double Rmax, long tipo, double K_el, double* Vpar, double *pot){
  double delta, R;
  long int i, j, bin;
  ...
  for(i=0;i<Nbin;i++){
    ...
    pot[i] = V_pot( 1, tipo, K_el, Vpar, &R );
    pot[i+Nbin] = R;
  }
}
