...
//per compilare: gcc -lm prog.c -o prog
...
int main(){
  ...
  for( n=0; n<Nsteps; n++ ){
    ...
    if(flag==1){
      ...
      if(n<=Tequil/tau && ((n % stepsperblock)==0)){  //costruisce la funzione d'onda solo nell'intervallo di equilibrazione
...
void GeneraWalkerG(long int D, long int Nw, double **w){
  ...
  for(i=0;i<Nw;i++){
    for(j=0;j<D;j++){
      //w[i][j] = norm(-2.0, 2.0/5.0);	//norm(0,1);
...
void GeneraWalkerU(long int D, long int Nw, double **w){
  ...
  for(i=0;i<Nw;i++){
    for(j=0;j<D;j++){
      w[i][j]=(rand()/(double)(RAND_MAX)-0.5);		//5*(rand()/(double)(RAND_MAX)-0.5);
    }
  }
}
